...
import {
	InMemoryRefreshTokenAdapter,
	InMemoryUserAdapter,
	InMemoryTokenServiceAdapter,
	InMemoryTokenCache,
...
describe("refresh token use case tests", () => {
	test("should generate new access token with valid refresh token", async () => {
		...
		vi.spyOn(tokenService, "verify").mockResolvedValue({
			tokenId: refreshToken.id!.value,
		} as any);
	...
	test("should throw error when refresh token is not provided", async () => {
		...
		await expect(useCase.perform(input)).rejects.toThrow(
	...
	test("should create refresh token with default expiration when not provided", async () => {
		...
		expect(
			Math.abs(refreshToken.expiresAt.getTime() - eightDaysFromNow.getTime()),
		).toBeLessThan(60000);
	});
	...
	test("should throw error when token is not in cache and invalid in DB", async () => {
		...
		await refreshTokenRepository.updateRevokedAt(
		...
		await expect(useCase.perform(input)).rejects.toThrowError(
			new InvalidCredentialsError("Credenciais inválidas ou expiradas", 401),
		);
	});
});
