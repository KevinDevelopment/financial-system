...
import { VerifyTokenInputDto, VerifyTokenOutputDto } from "../../../../core/application/dto";
import { MissingDataError, InvalidValueError } from "../../../../core/domain/errors";
...
describe("verify token use case test", () => {
    ...
    test("Should by pass in verify", async () => {
        const input = { token: "fake-token" };
        const spyMethodCreateVerify = vitest.spyOn(tokenService, "verify")
            .mockResolvedValue(mockTokenResponse)
        const result = await useCase.perform(input);

        expect(spyMethodCreateVerify).toHaveBeenCalledWith(
            "access",
            "fake-token"
        );
