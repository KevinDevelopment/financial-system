...
describe("entity user tests", () => {
    ...
    test("Should return an error if email is not a string", () => {
        ...
        expect(user).toThrowError(new BusinessRuleViolationError("Email inválido", 422));
    });
    ...
    test("Should return an error if email is not provided", () => {
        ...
        expect(user).toThrowError(new BusinessRuleViolationError("Email não pode ser vazio", 422))
    });
    ...
    test("Should return an error if name is not provided", () => {
        ...
        expect(user).toThrowError(new BusinessRuleViolationError("Nome não pode ser vazio", 422))
    });
    ...
    test("Should return an error if name is greater than 100 characters", () => {
        ...
        expect(user).toThrowError(new BusinessRuleViolationError("Nome deve ter entre 2 e 100 caracteres", 422));
    });
    ...
    test("Should return an error ir role is invalid", () => {
        ...
        expect(user).toThrowError(new BusinessRuleViolationError("Role inválida", 422))
    });
});