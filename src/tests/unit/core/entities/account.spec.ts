...
describe("entity account tests", () => {
	test("Should create a valid account if values are provided", () => {
		...
		expect(account.type.value).toEqual(AccountType.CHECKING);
		expect(account.initialBalance.toDecimal()).toEqual(100.5);
		expect(account.currentBalance.toDecimal()).toEqual(100.5);
...
describe("value object money tests", () => {
	test("Should create Money from string value", () => {
		...
		expect(money.toDecimal()).toEqual(100.5);
